...
mprint_solution(o,n,m,me,evalx, timex, iflag,x,f,g,pr,pp,acc,psize,iout);
if( save2file > 0)
    mprint_final(iflag,timex,evalx,maxtime,maxeval,param,iout1)
    mprint_solution(o,n,m,me,evalx, timex, iflag,x,f,g,pr,pp,acc,psize,iout1);
    mprint_final(iflag,timex,evalx,maxtime,maxeval,param,iout2)
    mprint_solution(o,n,m,me,evalx, timex, iflag,x,f,g,pr,pp,acc,psize,iout2);
end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Preparing solution information for return to main program
solution.f = f;
solution.g = g;
solution.x = x;
solution.time = timex;
...
if(abs(pf) <= 1.0e+10)
    if(abs(pr) <= 1.0e+5)
      ...
      if( o >= 2 ); fprintf(iout,' [%9i,%8i]   F(X):%19.8f   VIO:%13.6f   [%6i]\n',evalx, timex,pf,pr,psize); end
    else
      if( o <= 1 ); fprintf(iout,' [%9i,%8i]        F(X):%19.8f         VIO:%13.6e\n',evalx, timex,pf,pr); end
      if( o >= 2 ); fprintf(iout,' [%9i,%8i]   F(X):%19.8f   VIO:%13.6e   [%6i]\n',evalx, timex,pf,pr,psize); end
    end
else
    if(abs(pr) <= 1.0e+5)
        if( o <= 1 ); fprintf(iout,' [%9i,%8i]        F(X):%19.8e         VIO:%13.6f\n',evalx, timex,pf,pr); end
      if( o >= 2 ); fprintf(iout,' [%9i,%8i]   F(X):%19.8e   VIO:%13.6f   [%6i]\n',evalx, timex,pf,pr,psize); end
    else
        if( o <= 1 ); fprintf(iout,' [%9i,%8i]        F(X):%19.8e         VIO:%13.6e\n',evalx, timex,pf,pr); end
      if( o >= 2 ); fprintf(iout,' [%9i,%8i]   F(X):%19.8e   VIO:%13.6e   [%6i]\n',evalx, timex,pf,pr,psize); end
...
if( o <= 1 )
  ...
  for i=1:o
    if(abs(pg(m+i)) <= 1.0e+18)
        if(pg(m+o+i)<1)
          fprintf(iout,' f(%4i) = %34.15f \n',i,pg(m+i));
        else
          fprintf(iout,' f(%4i) = %34.15f \n',i,-pg(m+i));
        end
    else
        if(pg(m+o+i)<1)
          fprintf(iout,' f(%4i) = %34.6e \n',i,pg(m+i));
        else
          fprintf(iout,' f(%4i) = %34.6e \n',i,-pg(m+i));
        end  
    end 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
fprintf(ioutx,'#########################################################\n');
fprintf(ioutx,'### This file contains the history of  ALL  solutions ###\n');
fprintf(ioutx,'#########################################################\n');
fprintf(ioutx,'### SOLUTION FORMAT:     F(1:O)    G(1:M)    X(1:N)   ###\n');
fprintf(ioutx,'#########################################################\n');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf(ioutx,'#                                                   \n');
fprintf(ioutx,'#        MIDACO STARTING POINT (very first solution)\n'); %%% DUMMY FOR NOW
fprintf(ioutx,'#                                                   \n');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
else
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if( abs(dummy) < 100000000 )
    fprintf(ioutx,'%19.8f',dummy);
else
    fprintf(ioutx,'%19.6e',dummy);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
